...
const std::vector<Spot> &Quote::getSpots() const {
    return this->spots;
}

std::vector<std::tuple<long, double>> Quote::getTuples() {

    std::vector<std::tuple<long, double> > tuples;

    for (std::vector<Spot>::iterator it = spots.begin();
         it != spots.end();
         ++it) {
        tuples.emplace_back(it->getDate(), it->getClose());
    }
